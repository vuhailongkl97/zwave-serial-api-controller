...
void Board_IndicatorInitLed(led_id_t led)
{
  if (!m_indicator_initialized)
  {
     ...
     */
    CMU_CLOCK_SELECT_SET(LFA, ULFRCO);
    ...
    int16_t pin = Board_GetLedPin(led);
    /* Disable output and route LETIMER0 OUT0 to indicator LED location
     */
#ifdef ZWAVE_SERIES_800
    GPIO->LETIMERROUTE.ROUTEEN &= ~GPIO_LETIMER_ROUTEEN_OUT0PEN;
    GPIO->LETIMERROUTE.OUT0ROUTE = (GPIO->LETIMERROUTE.OUT0ROUTE & ~_GPIO_LETIMER_OUT0ROUTE_MASK) | (((pin << 16) & 0x000F0000) | (port & 0x00000003));
#else
    /* Deduce location from pin and port */
    uint8_t location;
    for(location = 0; location < 31; location++) {
      if((AF_LETIMER0_OUT0_PIN(location) == pin) && (AF_LETIMER0_OUT0_PORT(location) == port)) {
        break;
      }
    }
    /* Check if location was found before setting route. This should never 
     * happen but since PORT/PIN is configurable we need to be safe */
    if(location <= 31) {
      LETIMER0->ROUTEPEN &= ~LETIMER_ROUTEPEN_OUT0PEN;
      LETIMER0->ROUTELOC0 = (LETIMER0->ROUTELOC0 & ~_LETIMER_ROUTELOC0_OUT0LOC_MASK) | location;
    }
#endif
